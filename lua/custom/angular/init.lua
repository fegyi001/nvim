...
function M.toggle_between_html_and_ts()
  local current_buffer = vim.api.nvim_buf_get_name(0)
  local buf_path = path:new(current_buffer)
  local relative_path = buf_path:make_relative()
  local filename = string.match(relative_path, "([^/]+)$")
  ...
  if string.match(filename, "component.html") then
    -- if the current file is a spec file, then jump to the file it is testing
    local file_name = string.match(filename, "(.-)%.component")
    full_destination = buf_path:parent() .. "/" .. file_name .. ".component.ts"
  else
    -- if the current file is not a spec file, then jump to the spec file
    local filename_without_ext = string.match(filename, "(.-)%.component.ts")
    full_destination = buf_path:parent() .. "/" .. filename_without_ext .. ".component.html"
  end
